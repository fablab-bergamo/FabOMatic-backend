...
**Status (2026-09-06):** Fully remediated. File untracked (`git rm --cached`) and added to `.gitignore`; the Gmail app password was rotated by the repo owner; and git history across the whole repository (all branches and tags, not just `main`) was rewritten with `git filter-repo` to remove `settings.toml` entirely from every one of the 10 historical commits that touched it, plus a second pass to scrub the literal leaked password string from every blob (it had also been quoted verbatim in an earlier draft of this document). The rewritten history was force-pushed to `origin` for `main`, `authentik`, `development`, `dont-overwrite-settings`, `feature/sync-cache-interface`, `update-pkg`, and all tags.
**Residual exposure note:** GitHub retains its own internal `refs/pull/<N>/head` and `refs/pull/<N>/merge` refs for every PR ever opened against this repo, independent of branch/tag history — these cannot be rewritten by a normal force-push. Any merged PR whose diff included the old `settings.toml` content (at minimum PR #32, which introduced it) may still render the old blob content on GitHub's PR "Files changed"/"Commits" view even after this rewrite. Since the credential itself is rotated, this is now a residual-exposure/hygiene concern rather than a live credential leak; if you want it fully gone from GitHub's side, that requires contacting GitHub Support to purge cached PR data, or deleting/recreating the PRs in question.
